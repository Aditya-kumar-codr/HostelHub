...
const AdminStudentsData = () => {
  ...
  const handleDeleteStudent = async () => {
    ...
    if (!window.confirm(`Are you sure you want to delete ${selectedStudent.name} from the Students Directory?`)) {
      return;
    }
          ...
          {navigation.map((item) => {
            ...
            return (
              <button
                key={item.id}
                onClick={() => setActiveTab(item.id)}
                className={`flex items-center gap-2 px-3.5 py-2 rounded-lg text-sm font-bold transition-all whitespace-nowrap ${isActive
